...
var nicImageButton = nicEditorAdvancedButton.extend({	
	...
	submit : function(e) {
		...
		if(!this.im) {
			...
			console.log('im: '+JSON.stringify(this.inputs['src'].value));
		}
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
  ...
  uploadFile : function() {
    ...
    xhr.onload = function() {
    	var pardata = JSON.parse(xhr.responseText);
      console.log('responseText: '+JSON.stringify(pardata));
      try {
        var data = JSON.parse(xhr.responseText);
        this.onUploaded(data);
      } catch(e) {
      	console.log('try caught error'+data);
        return this.onError();
      }
      if(data.error) {
      	console.log('date error: '+data.error)
  ...
  onUploaded : function(options) {
    ...
    if(this.im) {
      this.im.setAttributes({
        src : src,
        width : w ,//(w && options.width) ? Math.min(w, options.width) : '',
        //height: 200
      });
    }
  }
